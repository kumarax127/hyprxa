...
class BasicAuthClient(AuthenticationClient):
    ...
    def __init__(self, authenticated_users: Set , unauthenticated_users: Set):
        self.authenticated_users = authenticated_users
        self.unauthenticated_users = unauthenticated_users
    ...
    def get_user(self, username: str) -> BaseUser:
        if username in self.authenticated_users:
            return BaseUser(username=username, scopes={"authenticated", "ADMIN"})
        elif username in self.unauthenticated_users:
            return BaseUser(username=username, scopes={"unauthenticated"})
...
def app(token_handler) -> FastAPI:
    ...
    auth_client = BasicAuthClient({"admin"}, {"anonymous"})
...
def test_invalid_username(test_client: TestClient, token_handler: TokenHandler):
